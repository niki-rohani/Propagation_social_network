 ...
 *
 */
public class CopyOfJacquartSimByPredecessor extends StrSim {

	// private static HashMap <String, Double> sim = new HashMap <String, Double> ();
	public CopyOfJacquartSimByPredecessor (DataUser data) {
		super (data);
	}
	...
	//		return sim.get(name);
		...
		Collection inter = u1.getPredList();
		Collection <Integer> union = u1.getPredList(); 
		inter.retainAll(u2.getPredList());
		union.addAll(u2.getPredList());
		if (union.size() == 0) {
	//		sim.put(name, 0.0);
			return 0.;
