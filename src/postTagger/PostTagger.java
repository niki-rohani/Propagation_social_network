...
public abstract class PostTagger {
	...
	public String tagCollection(String inputDB, String inputCol, boolean supOldTags,boolean removeText,boolean videMem){
		String outputCol=MongoDB.mongoDB.createCollection(inputDB,"posts"," posts from "+inputCol+" taggues selon "+this.toString()+" supOldTags="+supOldTags+", removeText="+removeText);
		...
		try {
			...
			while(cursor.hasNext()) {
				...
				if (p!=null){
					...
					p.indexInto(outputDB,outputCol);
			...
			DBCollection outcol=MongoDB.mongoDB.getCollectionFromDB(outputDB,outputCol);
