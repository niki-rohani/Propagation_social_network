...
public class CPULogistic extends Module
{
	...
	public CPULogistic(int _size, double K, double a, double lambda){
		size=_size;
	...
	public CPULogistic(int _size, double lambda){
		this(_size,1,1,lambda);
	}
	public CPULogistic(int _size)
	{
		
		this(_size,1,1,1);
	}
	public CPULogistic(CPULogistic org){
		this(org.size,org.K,org.alpha,org.lambda);
		this.origin_module=org;
	}

	public Module forwardSharedModule()
	{
		CPULogistic ret=new CPULogistic(this);
	...
	{
		return(new CPULogistic(size,K,alpha,lambda));
	}
	...
	public double getLValue(double x)
	{
 	 	double f=(double)(K/(1.0+alpha*Math.exp(-lambda*x)));
		 return(f);
	}

	public double getDLValue(double x)
	{
		double f=(double)(1.0/(1.0+alpha*Math.exp(-lambda*x)));
		
		return((double)(K*lambda*f*(1.0-f)));
	}
	...
	{	
		...
		for(int nbexample=0;nbexample<minibatch_size;nbexample++)
		{
			for(int idx_output=0;idx_output<size;idx_output++)
			{
				...
				out[Matrix.IDX2C(nbexample,idx_output,minibatch_size)]=getLValue(in[Matrix.IDX2C(nbexample,idx_output,minibatch_size)]);
			}
		}
	...
	public void backward_computeDeltaInputs(Tensor input,Tensor deltas_output)
	{
		...
		for(int nbexample=0;nbexample<minibatch_size;nbexample++)
		{
			for(int idx_input=0;idx_input<size;idx_input++)
			{
				...
				double val=getDLValue(in[idx])*o;
				if(Double.isNaN(val)){
					throw new RuntimeException(this+".computeDeltas: NaN val computed => "+o+" * "+getDLValue(in[idx]));		
				}
				d_in[idx]=val;
			}
		}		
	...
	{
		String r="CPULogistic_"+this.name+"("+K+","+alpha+","+lambda+"): "+size;
		return(r);
	}	
	...
	{
		//return(new GPULogistic(size,K,a,lambda));
		return null;
	}
