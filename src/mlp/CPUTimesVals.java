...
public class CPUTimesVals extends Module {
	...
	//protected ArrayList<CPUTimesVals> listeners;
	
	public CPUTimesVals(int size){
		this(size,(CPUMatrix)null);
	}
	
	public CPUTimesVals(int size,double val){
		this(size,new CPUMatrix(1,size,val));
	}
	
	public CPUTimesVals(int size,CPUMatrix vals){
		this.size=size;
		if(vals==null){
	...
	public Module forwardSharedModule()
	{
		...
		CPUTimesVals ret=new CPUTimesVals(this);
	...
	public String toString(){
		return "CPUTimesVals_name="+name+((sharedForward)?"_sharedForward":"");
	}
	...
	public void forward(Tensor input) {
		...
		//System.out.println(this+": allocate("+minibatch_size+")");
		...
		if(minibatch_size%vals.number_of_rows!=0){
			throw new RuntimeException(this+" Format pb : vals and input number of rows incompatible = "+vals.number_of_rows+","+minibatch_size);
		}
	...
	public void backward_computeDeltaInputs(Tensor input, Tensor deltas_output) {
		...
		int minibatch_size=input.getMatrix(0).getNumberOfRows();
		//System.out.println(this+": backward("+minibatch_size+") "+this.tensor_delta.getMatrix(0).getNumberOfRows());
		...
		{		
			...
			{
					double w=vals.getValue((int)(nbexample/n),idx_input);
