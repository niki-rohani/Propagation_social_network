...
public class CPUL1Norm extends Module {

	protected Tensor tensor_output;
	protected Tensor tensor_delta;
	protected int size;
	
	public CPUL1Norm(int s){
		size=s;
	...
	public CPUL1Norm(CPUL1Norm org)
	{
		this(org.size);
	...
	{
		CPUL1Norm ret=new CPUL1Norm(this);
	...
	public void forward(Tensor input) {
		...
		for(int nbexample=0;nbexample<minibatch_size;nbexample++)
		{
			...
			for(int idx_output=0;idx_output<size;idx_output++)
			{
				...
				out[Matrix.IDX2C(nbexample,0,minibatch_size)]+=Math.abs(f);
			}
		}		
