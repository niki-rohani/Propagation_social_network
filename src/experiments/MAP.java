...
import core.User;
public class MAP extends EvalMeasure {
	protected LinkedHashSet<String> allUsers;
	boolean ignoreInit;
	
	public MAP(HashSet<String> allUsers){
		this(allUsers, true);
	}
	
		
	public MAP(HashSet<String> allUsers, boolean ignoreInit){
	...
	public String getName() {
		return("MAP_"+((ignoreInit)?"_ignoreInit":""));
	}
		...
		for(String u:list){
			...
			if ((!firsts.containsKey(u)) && ((!ignoreInit) || (!init.containsKey(u)))){
				...
				if (href.containsKey(u)){
					nbRef++;
					System.out.print(nb+" ("+sumContaminated.get(u)+") ");
					sum+=(nbRef*1.0)/(nb*1.0);
